      ...
      <div className="relative flex w-full overflow-hidden">
        ...
        <div className="flex w-max animate-marquee items-center gap-6 sm:gap-10 hover:[animation-play-state:paused]">
          {scrollItems.map((brand, i) => (
            <div
              key={`${brand.name}-${i}`}
              className="group flex h-24 w-40 items-center justify-center rounded-2xl border border-border bg-card p-6 shadow-sm transition-all duration-300 hover:border-primary/20 hover:shadow-xl hover:shadow-primary/5 sm:h-28 sm:w-48"
            >
              <img
                src="/logo.webp"
