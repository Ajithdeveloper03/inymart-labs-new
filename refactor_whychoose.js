...
for (const relPath of filesToProcess) {
  ...
  let headingSectionMatch = content.match(/<div className="text-center max-w-3xl mx-auto mb-16">([\s\S]*?)<\/div>/);
  let headingSection = headingSectionMatch ? headingSectionMatch[1] : '';

  // Extract Cards
  const cardRegex = /<Reveal[^>]*>[\s\S]*?<div[^>]*>[\s\S]*?<([A-Z][a-zA-Z0-9]*)[^>]*\/>[\s\S]*?<h3[^>]*>([\s\S]*?)<\/h3>[\s\S]*?<p[^>]*>([\s\S]*?)<\/p>[\s\S]*?<\/div>[\s\S]*?<\/Reveal>/g;
  let cards = [];
  let match;
  ...
  if (cards.length === 0) {
    console.log('No cards found in', relPath, 'trying alternative regex');
    // Maybe the original didn't have Reveal around it, or icon was different. But we know they were generated similarly.
    const altRegex = /<([A-Z][a-zA-Z0-9]*)\s+className="w-12 h-12[^>]*\/>[\s\S]*?<h3[^>]*>([\s\S]*?)<\/h3>[\s\S]*?<p[^>]*>([\s\S]*?)<\/p>/g;
  ...
  const gridClass = cards.length === 2 ? "grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 max-w-4xl mx-auto relative" : "grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 max-w-6xl mx-auto relative";

  let newCardsHtml = cards.map((card, i) => `          {/* Card ${i + 1} */}
